<template>
  <div class="team-report">
    <div class="header">
      <h1>{{ reportData?.team?.name }} - {{ period }} Report</h1>
    ...
    <div v-if="!loading && !error && reportData" class="report-content">
      <div class="report-section">
        ...
        <div class="info-grid">
          <div class="info-item">
            ...
            <span>{{ reportData.team?.name }}</span>
          </div>
          <div class="info-item">
            <label>Members Count:</label>
            <span>{{ reportData.team?.members_count }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="report-section">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            <h3>In Progress</h3>
            <p class="stat-value">{{ reportData.projects?.in_progress || 0 }}</p>
          </div>
        </div>
      </div>
...
const exportPDF = async () => {
  try {
    const params = {
      period: period.value,
      week_start: filters.value.week_start,
      week_end: filters.value.week_end,
      month: filters.value.month,
      year: filters.value.year
    }
    const response = await exportService.exportTeamReportPDF(teamId.value, params)
    exportService.downloadBlob(response.data, `team-report-${teamId.value}-${period.value}.pdf`)
  } catch (err) {
...
const exportCSV = async () => {
  try {
    ...
    const response = await exportService.exportTeamReportCSV(teamId.value, params)
    exportService.downloadBlob(response.data, `team-report-${teamId.value}-${period.value}.csv`)
  } catch (err) {
    toast.showError(getErrorMessage(err, 'Failed to export CSV'))
  }
}
...
const loadReport = async () => {
  ...
  try {
    ...
    if (period.value === 'weekly') {
      ...
      response = await reportService.getWeeklyTeamReport(teamId.value, params)
    } else if (period.value === 'monthly') {
      if (filters.value.month) params.month = filters.value.month
      if (filters.value.year) params.year = filters.value.year
      response = await reportService.getMonthlyTeamReport(teamId.value, params)
    } else if (period.value === 'yearly') {
      if (filters.value.year) params.year = filters.value.year
      response = await reportService.getYearlyTeamReport(teamId.value, params)
    }
